...
type ProviderSearchResult = {
  ...
  snippets: Array<{ snippet: string; url: string }>;
};
...
async function searchDuckDuckGo(query: string): Promise<ProviderSearchResult> {
  ...
  data.Results?.forEach((r) => { if (r.Text) snippets.push({ snippet: r.Text, url: r.FirstURL || "" }); });
  data.RelatedTopics?.forEach((t) => {
    if (t.Text) snippets.push({ snippet: t.Text, url: t.FirstURL || "" });
    t.Topics?.forEach((st) => { if (st.Text) snippets.push({ snippet: st.Text, url: st.FirstURL || "" }); });
  });
...
async function checkSentenceOnWeb(
  ...
  for (const snippetObj of snippets) {
    const snippetWords = normalizeWords(snippetObj.snippet);
    const overlap = calculateWordOverlapRatio(targetWords, snippetWords);
    if (overlap > bestOverlap) {
      bestOverlap = overlap;
      if (snippetObj.url && snippetObj.url.startsWith("http")) matchingUrl = snippetObj.url;
    }
  ...
  return {
    ...
    successfulProviders,
    failedProviders,
    matchedUrls: bestOverlap >= 0.4 && matchingUrl ? [matchingUrl] : []
  };
}
    ...
    if (sentences.length > 0) {
      ...
      for (const result of results) {
        if (result.status === "fulfilled") {
          ...
          if (result.value.matchedUrls) {
             for (const url of result.value.matchedUrls) allMatchedUrls.add(url);
          }
